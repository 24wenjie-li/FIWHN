...
class SRData(data.Dataset):
    ...
    def _load_file(self, idx):
        ...
        if self.ext1 == '.npy':
            lr = [npy_loader(f_lr[idx_scale]) for idx_scale in range(len(self.scale))]
            hr = npy_loader(f_hr)
        if not self.train:
            hr = default_loader(f_hr)
            lr = [default_loader(f_lr[idx_scale]) for idx_scale in range(len(self.scale))]
...
class SRData1(data.Dataset):
    ...
    def _set_filesystem(self, data_dir):
        self.apath = os.path.join(data_dir, 'DIV2K')
        self.dir_hr = os.path.join(self.apath, 'DIV2K_train_HR')
        self.dir_lr = os.path.join(self.apath, 'DIV2K_train_LR_bicubic')
        self.ext = ('.png', '.png')
